...
exports.Crypto = {};
exports.MD5 = exports.Crypto.MD5 = function (data, callback) {
  if (typeof data === "string") {
    if (!process.browser) {
      callback(crypto.createHash('md5').update(data).digest('hex'));
    } else {
      callback(md5.hash(data));
    }
    return;
  }
  var chunkSize = process.browser ? 524288 : data.size;
  var blobSlice = File.prototype.slice ||
                  File.prototype.mozSlice ||
                  File.prototype.webkitSlice;
  var chunks = Math.ceil(data.size / chunkSize);
  var currentChunk = 0;
  ...
  var onload = function (e) {
    ...
    if (currentChunk < chunks) {
      ...
      if (process.browser) {
        ...
        var binary = utils.arrayBufferToBinaryString(e.target.result);
        callback(crypto.createHash('md5').update(binary).digest('hex'));
      }
      buffer.destroy();
    }
  };
  var loadNextChunk = function () {
    ...
    reader.readAsArrayBuffer(blobSlice.call(data, start, end));
  };
  loadNextChunk();
};
